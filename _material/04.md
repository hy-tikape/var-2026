...
Observera att eftersom filmen Dumbo saknar utgivningsår så dyker den inte upp i något av svaren. Vi kan hämta de filmer som saknar utgivningsår med följande fråga: 
...
Ovanstående exempel är ett typiskt sätt att använda `IFNULL(a, b)` på. Funktionen omvandlar ett eventuellt `NULL`-värde till noll när den andra parametern (`b`) i `IFNULL(a, b)` är 0. Detta är användbart till exempel i `LEFT JOIN`-frågor tillsammans med `SUM`-funktionen.
...
Vi kan formulera underfrågor på samma sätt som vanliga frågor och använda dem för att göra sökningar som annars vore svåra att genomföra.
