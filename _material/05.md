...
I exemplen i det här kapitlet och i kursuppgifterna används en testdatabas som innehåller uppgifter om cykelturer med stadscyklarna under år 2024 i Helsingfors och Esbo. Databasen bygger på [offentlig data](https://www.avoindata.fi/data/fi/dataset/helsingin-ja-espoon-kaupunkipyorilla-ajatut-matkat) som publicerats av HRT. Datan har bearbetats till en SQL-databas för denna kurs.
...
* `id`: ID-nummer
* `start_time`: cykelturens starttid (i formen yyyy-mm-ddThh:mm:ss)
* `end_time`: cykelturens sluttid (i formen yyyy-mm-ddThh:mm:ss)
* `start_station_id`: ID-nummer för startstationen
* `end_station_id`: ID-nummer för slutstationen
* `distance`: cykelturens längd (i meter)
* `duration`: cykelturens varaktighet (i sekunder)
...
En svaghet med koden är att den inte tar hänsyn till situationer där det i databasen inte finns någon station med det angivna ID-numret.
...
Följande kod frågar användaren efter en startstation och ett datum, och söker sedan upp alla slutstationer för den angivna dagen dit cykelturer som började vid startstationen avslutades.
