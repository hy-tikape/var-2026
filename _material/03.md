...
I fall 1 kan vi lägga till en kolumn i tabell A som refererar till tabell B precis som vi gjorde i exemplet i föregående avsnitt. Fall 2 är däremot mer komplicerad eftersom en enskild referens i någon av tabellraderna inte räcker. Lösningen är att skapa en tredje _kopplingstabell_ (_join table_) som innehåller informationen om referenserna.
...
Syftet med frågan är att hämta information från tabellerna `Customers` och `Products` som motsvarar raderna i tabellen `Purchases`. För att få meningsfulla resultat kopplar vi ihop raderna med hjälp av två villkor. Frågan ger som svar följande resultattabell:
...
### Fler villkor i frågan

Vi kan lägga till fler villkor i frågan om vi vill få reda på mer om köpkorgarna. Till exempel hämtar följande fråga de produkter som finns i Maijas köpkorg:
...
Om vi lägger till grupperingen `GROUP BY Customers.id` i frågan delas raderna upp i två grupper baserat på kolumnen `Customers.id`:
...
I exemplet ovan tillför JOIN-syntaxen inget nytt, utan är bara ett alternativt sätt att ställa frågan på. Vi ska i nästa exempel sehur vi kan utvidga syntaxen så att den ger oss nya möjligheter i frågorna.
...
Lösningen på problemet är att använda `LEFT JOIN`-syntaxen, vilket innebär att om en rad i den vänstra tabellen inte kopplas till någon rad i den högra tabellen, inkluderas den vänstra raden ändå som en rad i resultattabellen. För den raden får varje kolumn som baseras på den högra tabellen värdet `NULL`.
...
Faktum är att det finns hela fyra olika varianter av JOIN-frågan:
...
En begränsning i SQLite är att endast de två första frågetyperna ovan är möjliga. Som tur är brukar `LEFT JOIN` vara just den frågetyp vi behöver.
...
I det här fallet gör `WHERE`-delen att resultattabellen inte innehåller någon rad för kursen är Datanätverk.

Om båda villkoren istället finns i `ON`-delen, förändras resultatet igen:
...
Vi återgår till det tidigare problemet där frågan som sammanfattar saknade information. I vår databas finns fortfarande följande tabeller:
...
I det andra steget slås resultattabellen ihop från det första steget med tabellen `Products`:
