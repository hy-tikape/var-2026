...
Kommandot `SELECT` ställer en _fråga_ (_query_), alltså hämtar data från en tabell. Det enklaste sättet att formulera en fråga på är att hämta all data från tabellen:
...
Låt oss betrakta en exempelfråga som innehåller alla ovannämnda delar samtidigt. Frågan körs mot tabellen `Tasks` som innehåller uppgifter kopplade till olika projekt. Varje uppgift har en viss prioritet. En uppgift är _kritisk_ om dess prioritet är minst 3.
