...
I SQLite-tolken gäller som standard att databasen finns i minnet (en så kallad _in-memory database_). Det betyder att databasen från början är tom och försvinner när tolken stängs. Detta är ett bra sätt att testa olika egenskaper i SQL-språket. En diskussion med tolken kan exempelvis se ut såhär:
...
Vi lägger sedan till rader i tabellen och efter det hämtar vi raderna från tabellen. SQLite-tolken visar resultatet med lodräta streck mellan kolumnerna, vilket kan vara en aning svårt att läsa. Utskriften kan förbättras med tolkens kommandon `.mode column` (varje kolumn får en fast bredd) och `.headers on` (kolumnnamnen visas).
