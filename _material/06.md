...
Låt oss som exempel betrakta en situation där en tabellkolumn innehåller ett användarnamn. Samma kolumn innehåller både för- och efternman. Är detta en dålig design?
...
* Användaren ser salens kod, byggnaden, antalet sittplatser och priset. Exempel: A111, Exactum, 280, A10
* Timpriset för bokningen beror på prisnivån. Användaren ser det totala priset i samband med bokningen.
...
En bra lösning är att skapa separata tabeller `Buildings` och `Categories` där information om byggnader respektive prisnivåer sparas:
...
För denna funktion kan vi lägga till en kolumn `price_per_hour` i tabellen `Categories` som anger timpriset för respektive prisnivå:
